    ...
    >
      <template v-slot:header="props">
        ...
        <q-tr :props="props" class="ignore-elements" v-if="columns_filter">
          ...
          >
                  ...
                  <q-item-section avatar>
                    <q-checkbox
                      @update:model-value="
                        ...
                        column_options_selected[col.field].length
                          ? (column_options_selected[col.field] = [])
                          : (column_options_selected[col.field] = getColumnOptions(
                              col.field
                            ).map((item) => item.value))
                      "
                      :model-value="
            ...
            <q-input
              v-if="col.hasOwnProperty('filter_type') && col.filter_type == 'date'"
              dense
              color="teal"
              class="q-pl-xs q-pr-xs"
              filled
      ...
      <template v-slot:body="props">
        <q-tr :props="props" v-if="!hasDefaultSlot" @click="rowClick(props.row)">
          ...
          <q-td v-for="(col, col_index) in props.cols" :key="col.name" :props="props">
            <q-btn
              size="sm"
              color="accent"
              round
              ...
              v-if="
                groupby_filter && selected_group_by_filed.value != '' && col_index == 0
              "
            />
...
export default defineComponent({
    ...
    getFilteredData() {
      ...
      let table_Data = this.data.filter(function (item) {
        ...
        for (i = 0; i < table_columns.length; i++) {
          ...
          if (item[table_columns[i]] == null) return true;
  ...
  methods: {
    ...
    setColumnsDefinition() {
      ...
      this.final_column =
        this.selected_group_by_filed.value != "" ? this.grouped_column : this.columns;
    },
    ...
    exportTable(type) {
      ...
      const content = [this.columns.map((col) => wrapCsvValue(col.label))]
        ...
        .join("\r\n");
      const status = exportFile(this.file_name + "." + type, content, "text/" + type);
      if (status !== true) {
        this.$q.notify({
